...
function get_shape_info(inarr_shape: DTYPE_t[]): shape_info {
	...
	const good_shape: DTYPE_t[] = inarr_shape.slice().sort();
	...
	if (res.ndim === 1) {
		...
		if (res.x === 1 && res.y > 1) {
			throw new Error(
				`Swap axis of your ${inarr_shape} array so it has a ${good_shape} shape`,
			);
		}
	} else if (res.ndim === 3) {
		res.x = inarr_shape[2];
		...
		if ((res.x === 1 && res.y > 1) || (res.y === 1 && res.z > 1)) {
			throw new Error(
				`Swap axes of your ${inarr_shape} array so it has a ${good_shape} shape`,
			);
		}
	} else {
		throw new Error(
			`Only for images of dimension 1-3 are supported, got a ${res.ndim}D one`,
		);
	}
